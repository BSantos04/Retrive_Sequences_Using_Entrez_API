...
def parse_arguments():
    """
    Before starting retrieving the sequences, we first need to parse the command line arguments to prevent any future misuse from the user.
    ...
    In that case, we want to attach the database name and the term name.
    
    Finally, if there was an error in the command line, the function will return a message specifying the missing argument(s) and close the program.
...
def search_sequences(database,term):
    """
...
def fetch_sequences(database,webenv,querykey):
    """
...
def main():
    ...
    webenv, querykey = search_sequences(database,term)
    fetch_sequences(database,webenv,querykey)
